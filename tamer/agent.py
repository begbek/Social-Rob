...
CARTPOLE_ACTION_MAP = {0: '< left', 1: 'right >', 2: 'Pole_NONE'}
ACTION_MAPS = {"CartPole-v1" : CARTPOLE_ACTION_MAP, 'MountainCar-v0' : MOUNTAINCAR_ACTION_MAP}
...
class Tamer:
    ...
    def __init__(
        ...
        env_name = 'NONE', # name of the environment
        imper = False # if True, gets imperative feedback for the agent's action
    ):
        self.tame = tame
        self.ts_len = ts_len
        self.env = env
        ...
        self.action_map = ACTION_MAPS[env_name]
        self.imper = imper
            ...
            for ts in count():
                ...
                if not self.tame:
                    ...
                    while time.time() < now + self.ts_len:
                        ...
                        human_reward = disp.get_emotional_feedback()
                        ...
                        if human_reward != 0:
                            ...
                            print(state,' & ',action,' & ',human_reward)
    ...
    async def train(self, model_file_to_save=None):
        ...
        self.env.render()
        disp = None
        if self.tame or self.imper:
            # only init pygame display if we're actually training tamer
            from .interface import Interface
            disp = Interface(action_map=self.action_map, tame=self.tame, imper=self.imper)
